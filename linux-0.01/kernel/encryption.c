...
struct encrypted_file encrypted_files[1024];
int current_encrypted_files = 0;
const int HASH_SEED = 12;

int add_to_encrypted_list(unsigned short i_uid){
	current_encrypted_files++;
	encrypted_files[current_encrypted_files].i_num = i_uid;
	int hash = MurmurHash2(global_key,strlen(global_key),12);
	encrypted_files[current_encrypted_files].key_hash = hash;
	printk("File added to encrypted list with key hash: %d \n",encrypted_files[current_encrypted_files].key_hash);
	return 1;
}
...
int is_current_key_valid_for_file(unsigned short i_uid){
	...
	int i;
	int r = 0;
	for(i = 0; i < current_encrypted_files+1;i++){
		//printk("Comparing %d with %d \n",encrypted_files[i].key_hash,hash);
		if(encrypted_files[i].i_num == i_uid){
			printk("File found\n");
			// file found in encrypted list			
			if(encrypted_files[i].key_hash == hash){
				r = 1;
			}
			printk("File key hash: %d\nCurrent key hash:%d\n",encrypted_files[i].key_hash,hash);			
			break;
		}
...
int remove_from_encrypted_list(unsigned short i_uid){
	int i;
	struct encrypted_file null_struct = {0};
	for(i = 0; i < current_encrypted_files;i++){
		if(encrypted_files[i].i_num == i_uid){
			encrypted_files[i] = null_struct;
			break;
		}
	}
	return 0;
...
int sys_encrypt(int fd){
	...
	add_to_encrypted_list(inode->i_num);
